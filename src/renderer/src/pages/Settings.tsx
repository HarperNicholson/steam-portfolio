...
export default function Settings(): JSX.Element {
  const { settings, setSetting, loadSettings, accounts, addToast, isImportingHistory, importHistory } = useStore()
  const [sessionCookie, setSessionCookie] = useState(settings['session_cookie'] ?? '')
  const [removing, setRemoving] = useState<number | null>(null)
  const [appVersion, setAppVersion] = useState<string>('')

  useEffect(() => {
    window.sp.app.version().then(setAppVersion).catch(() => {})
  }, [])
  ...
  return (
    <div className={styles.page}>
      ...
      <Section title="Default Alert Settings">
        <p className={styles.hint}>
          Applied to items with no individual alert configuration.
          Per-item overrides (set in item detail) always take precedence.
        </p>
        <div className={styles.row}>
          <label className={styles.label}>
            <input
        ...
        <div className={styles.row}>
          <span className={styles.label} style={{ cursor: 'default' }}>Default peak drop threshold:</span>
          <input
            type="range"
            min="5"
            max="50"
            ...
            style={{ width: 120 }}
          />
          <span style={{ fontSize: 13, color: 'var(--text-primary)', minWidth: 36 }}>
            {Math.round(parseFloat(settings['default_ath_drop_threshold'] ?? '0.1') * 100)}%
          </span>
        </div>
      </Section>
      ...
      <Section title="Smart Range">
        <p className={styles.hint}>
          New items often spike in price due to scarcity when first released.
          Smart Range skips the first <strong>N days</strong> of price history when computing the peak price,
          giving a more realistic baseline for drop alerts.
          Set to 0 to disable (uses standard All-Time High).
        </p>
        <div className={styles.row}>
          ...
          />
          <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>days (0 = off)</span>
        </div>
        <p className={styles.hint}>Takes effect on the next price refresh.</p>
      </Section>
      ...
      <Section title="System">
        <div className={styles.row}>
          <label className={styles.label}>
            <input
              ...
              onChange={async (e) => {
                await window.sp.app.setAutostart(e.target.checked)
                await loadSettings()
              }}
            />
            Launch at login
        ...
        <div className={styles.row}>
          <label className={styles.label}>
            <input
              ...
              onChange={async (e) => {
                await window.sp.app.setTray(e.target.checked)
                await loadSettings()
              }}
            />
        ...
        {settings['minimize_to_tray'] === '1' && (
          <p className={styles.hint}>
            Closing the window hides the app to the system tray. Click the tray icon to restore, or right-click to quit.
          </p>
        )}
      </Section>
      ...
      <Section title="About">
        <p className={styles.hint}>
          SteamPortfolio {appVersion ? `v${appVersion}` : ''} · MIT License
        </p>
        <p className={styles.hint}>
