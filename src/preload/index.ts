...
const api = {
  ...
  app: {
    ...
    setAutostart: (enabled: boolean): Promise<{ ok: boolean }> => ipcRenderer.invoke('app:set-autostart', enabled),
    setTray: (enabled: boolean): Promise<{ ok: boolean }> => ipcRenderer.invoke('app:set-tray', enabled)
  },
  on: {
